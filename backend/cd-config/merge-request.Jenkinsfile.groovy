...
def branchToBuild = env.gitlabBranch == null ? (env.gitlabSourceBranch == null ? 'master' : env.gitlabSourceBranch) : env.gitlabBranch
println "Building $branchToBuild"
...
gitlabBuilds(builds: ['Initialize', 'Checkout', 'Build [Backend]', 'Test [Backend]', 'Lint [Backend]', 'Build [Frontend]', 'Test [Frontend]', 'Lint [Frontend]', 'Reporting']) {
  ...
  if (branchToBuild != 'master' && env.gitlabTargetBranch == null) {
    println "Skipping intermediate commit on branch: ${env.gitlabBranch}"
    return
    //actually do nothing in intermediate commit. Start build only when merge request submitted or pushing to master branch
  }
    ...
    node(label) {
      ...
      stage("Checkout SCM") {
        gitlabCommitStatus(name: "Checkout") {
          ...
          def extensions = env.gitlabMergeRequestId != null ? [[$class: 'UserIdentity', name: 'jenkins', email: 'nikita@rmcsoft.com'], [$class: 'PreBuildMerge', options: [fastForwardMode: 'NO_FF', mergeRemote: 'origin', mergeStrategy: 'DEFAULT', mergeTarget: "${env.gitlabTargetBranch}"]]] : []
          def userconfig = [credentialsId: 'e4e81238-d10a-42be-8f93-91280c0707e3', name: 'origin', url: 'git@practice.relex.ru:internal/aim.git']
          def configs = extensions.length != 0 ? [userconfig, userconfig] : [userconfig];

          checkout changelog: true, poll: true, scm: [
              $class                           : 'GitSCM',
              branches                         : [[name: "origin/${branchToBuild}"]],
