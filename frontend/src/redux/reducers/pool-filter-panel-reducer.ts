...
} from '../actions/pool-filter-panel-actions';
...
import { resetPoolsFilters, updatePoolsFilters } from '../actions/pool-filter-panel-actions';
...
const filters = (state: CustomFilterCmpContext = getDefaultFilters(), action: RootAction) => {
  switch (action.type) {
    case getType(updatePoolsFilters):
      return action.payload;
    case getType(resetPoolsFilters):
...
const poolFilterPanel = combineReducers({
  resourceTypeSuggestion,
  resourceTypeParameters,
  filters,
});

export default poolFilterPanel;
